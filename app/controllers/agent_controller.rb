class AgentController < ApplicationController
  def index
    # Step 1: Get only attributes associated with Agent
    @associated_attrs = AssociatedAttr.select do |attr|
      attr.associated_endpoints.include?("Agent")
    end
  
    # Step 2: Collect all unique product keys specifically for Agent
    ...
    if params[:products].present?
      ...
      when 'and'
        @associated_attrs = @associated_attrs.select do |attr|
          attr_products = attr.product_info['Agent']&.keys || []
    ...
    # Step 4: Search functionality (only within attributes already associated with Agent)
