class SellertransactionController < ApplicationController
  def index
    # Step 1: Get only attributes associated with SellerTransaction
    @associated_attrs = AssociatedAttr.select do |attr|
      attr.associated_endpoints.include?("SellerTransaction")
    end

    # Step 2: Collect all unique product keys specifically for SellerTransaction
    ...
    if params[:products].present?
      ...
      when 'and'
        @associated_attrs = @associated_attrs.select do |attr|
          attr_products = attr.product_info['SellerTransaction']&.keys || []
    ...
    # Step 4: Search functionality (only within attributes already associated with SellerTransaction)
