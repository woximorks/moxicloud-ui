class GalleryController < ApplicationController
  def index
    # Step 1: Get only attributes associated with Gallery
    @associated_attrs = AssociatedAttr.select do |attr|
      attr.associated_endpoints.include?("Gallery")
    end

    # Step 2: Collect all unique product keys specifically for Gallery
    ...
    if params[:products].present?
      ...
      when 'and'
        @associated_attrs = @associated_attrs.select do |attr|
          attr_products = attr.product_info['Gallery']&.keys || []
    ...
    # Step 4: Search functionality (only within attributes already associated with Gallery)
