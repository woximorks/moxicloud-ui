class ListingController < ApplicationController
  def index
    ...
    # Collect all unique product keys specifically for Listing
    @product_keys = @associated_attrs.flat_map { |attr| attr.product_info['Listing']&.keys }.compact.uniq

    # If the user selected products, filter the associated attributes based on Listing products only
    if params[:products].present?
      selected_products = params[:products]
      
      # Select attributes where any of the selected products exist under Listing
