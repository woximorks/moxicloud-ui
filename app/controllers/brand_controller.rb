class BrandController < ApplicationController
  def index
    # Step 1: Get only attributes associated with Brand
    @associated_attrs = AssociatedAttr.select do |attr|
      attr.associated_endpoints.include?("Brand")
    end

    # Step 2: Collect all unique product keys specifically for Brand
    ...
    if params[:products].present?
      ...
      when 'and'
        @associated_attrs = @associated_attrs.select do |attr|
          attr_products = attr.product_info['Brand']&.keys || []
    ...
    # Step 4: Search functionality (only within attributes already associated with Brand)
